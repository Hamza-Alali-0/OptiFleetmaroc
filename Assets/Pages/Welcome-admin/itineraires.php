...
$sql="select * from itineraires where ( localisations='$localisation');";
...
if (mysqli_num_rows($res) > 0) {
  ...
  if($localisation==isset($row['localisations']))
  {
    $email_error = " Localisation ALREADY EXISTS";
  }
}else{

  
    $stmt = $conn->prepare("insert into itineraires values(?)");
    $stmt->bind_param("s",$localisation);
    $stmt->execute();
