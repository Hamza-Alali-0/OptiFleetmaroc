...
$conn = mysqli_connect("localhost", "root", "", "optifleetmaroc") or die(mysqli_error());
...
$etat = $_POST['etat'];
    ...
    $sql_check_existence = "SELECT * FROM vehicule WHERE matricule='$id'";
    $result_existence = mysqli_query($conn, $sql_check_existence);

    if (mysqli_num_rows($result_existence) > 0) {
        $row = mysqli_fetch_assoc($result_existence);
        if ($id != $row['matricule']) {
            $email_error = "Matricule Doesn't exist";
        } else {
            $sql_check_email = "SELECT * FROM vehicule WHERE matricule='$matricule'";
            $result_email = mysqli_query($conn, $sql_check_email);

            if (mysqli_num_rows($result_email) > 0 && $id != $matricule) {
                $email_error = "Matricule existe deja";
            } else {
                $stmt = $conn->prepare("UPDATE vehicule SET matricule=?, marque=?, conducteur=?, localisation=?, etat=? WHERE matricule=?");
                $stmt->bind_param("ssssss", $matricule, $marque, $conducteur, $localisation, $etat, $id);
                $stmt->execute();
      ...
      <div class="container">
        <div class="row">
          <div class="col-md-12 text-center ftco-animate">
            <h2 class="text-uppercase ftco-uppercase">Modifier le Vehicule</h2>
        ...
        <div class="row">
          <div class="col-md bg-white p-5 m-2 text-center mb-2 ftco-animate">
            ...
            <legend>Entrer matricule du Vehicule a modifier </legend>
            ...
            <legend>Entrer marque </legend>
              ...
              <br><br>
              <legend>Entrer etat (libre/occupée) </legend>
