...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($result_existence->num_rows > 0) {
        ...
        if ($m != $row['matricule']) {
            ...
            // Insert data into etatvehicule table
            $sql_insert = "INSERT INTO etatvehicule (idvehicule, composants, huile, liquide, kilometrage) VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql_insert);
            $stmt->bind_param("ssssi", $m, $c, $h, $l, $k);
            if ($stmt->execute()) {
                $email_error = "Ajouté avec succès";
            } else {
                $email_error = "Erreur lors de l'ajout";
            }
            $stmt->close();
        }
    } else {
        $email_error = "Matricule non trouvé";
    }
}
      ...
      <div class="container">
        <div class="row">
          <div class="col-md-12 text-center ftco-animate">
            <h2 class="text-uppercase ftco-uppercase">Etat du Vehicule</h2>
